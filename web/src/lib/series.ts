...
export function hourlyView(hourly: HourTotals[]): HourlyView {
  ...
  return {
    ...
    note: hourlyNote(active, start),
  };
}

function hourlyNote(active: HourTotals[], start: number): string {
  ...
  const lead =
    `Busiest at ${formatHour(busiest.hour)}: ${formatUsd(busiest.succeeded_total_cents)} ` +
