...
from app.domain.currency import USD
...
def test_summary_narration_sends_facts_json() -> None:
    ...
    assert narrate_summary(llm, facts, USD) == "Quiet day so far."
...
def test_narrators_receive_dollars_never_cents() -> None:
    ...
    narrate_summary(llm, build_daily_facts([payment], [], now), USD)
    ...
    narrate_result(
        ...
        result={"amount_cents": 4500, "refund_id": "re_1"}, currency=USD,
    )
...
def test_summary_narration_names_the_weekday_so_the_model_never_guesses_it() -> None:
    ...
    llm = ScriptedLLM(["ok"])
    narrate_summary(llm, build_daily_facts([], [], datetime(2026, 9, 2, 15, tzinfo=UTC)), USD)
    payload = json.loads(llm.calls[0][1][0].content)
